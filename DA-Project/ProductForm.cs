        ...
        public void RefreshListView()
        {
            ...
            foreach (Product p in WarehouseEnt.Products)
            {
                ...
                foreach (ProductUnit pu in WarehouseEnt.ProductUnits.Where(pu => pu.Pcode == p.Pcode))
                {
                    ...
                    string[] WRow = { p.Pcode.ToString(), p.P_Name, p.Production_date.ToString("yyyy-MM-dd"), p.Expiration_date.ToString("yyyy-MM-dd"), p.Supplier_ID.ToString(), s.Supplier_Name, pu.Unit };
